...
public class SkinnedShadowVolume : MonoBehaviour
{
	...
	protected void SetShadowMaterials()
	{
		...
		if (backend == ShadowVolumeBackend.StencilBuffer)
		{
			if (isSimple)
			{
				renderer.sharedMaterials = new Material[] { stencilFrontBack };
			}
			else
			{
				renderer.sharedMaterials = new Material[] { stencilBackFrontAlways, stencilFrontBack };
			}
		}
		else if (backend == ShadowVolumeBackend.StencilBufferNoTwoSided)
		{
			if (isSimple)
			{
				renderer.sharedMaterials = new Material[] { stencilFront, stencilBack };
			}
			else
			{
				renderer.sharedMaterials = new Material[] { stencilBackAlways, stencilFrontAlways, stencilFront, stencilBack };
			}
		}
		else if (backend == ShadowVolumeBackend.AlphaChannel)
		{
			if (isSimple)
			{
				renderer.sharedMaterials = new Material[] { alphaFront, alphaBack };
			}
			else
			{
				renderer.sharedMaterials = new Material[] { alphaBackAlways, alphaFrontAlways, alphaFront, alphaBack };
			}
		}
		else if (backend == ShadowVolumeBackend.AlphaChannelNoBlendOp)
		{
			if (isSimple)
			{
				renderer.sharedMaterials = new Material[] { alphaFront, alphaBackNoBlendOp };
			}
			else
			{
				renderer.sharedMaterials = new Material[] { alphaBackAlways, alphaFrontAlwaysNoBlendOp, alphaFront, alphaBackNoBlendOp };
			}
		}
	}
