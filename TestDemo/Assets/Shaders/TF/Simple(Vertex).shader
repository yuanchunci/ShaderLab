Shader "kokichi/TF/Vertex Lighting" {
    Properties {
        ...
        Tags { "RenderType" = "Opaque" "LIGHTMODE"="ForwardBase" }
 		Pass 
 		{
			...
			struct v2f
			{
				...
				fixed diffuse : TEXCOORD1;
				fixed spec : TEXCOORD3;
			...
			fixed4 frag(v2f input) : COLOR
			{
				...
				fixed3 diffuseWarp = tex2D(_RampTex, fixed2(input.diffuse));
				fixed3 diffuseLight = diffuseWarp * _LightColor0.rgb;
				
				fixed4 specMask = tex2D(_SpecularMask, input.tex);
				fixed3 specLight = input.spec * _LightColor0.rgb * specMask.r;
				
				fixed3 rimLight = input.rim * specMask.g;
