...
half4 CalculateLight (v2f i)
{
	...
	#if defined (DIRECTIONAL_COOKIE) || defined(SUNSHINE_DISABLED)
	float atten = ComputeShadow (wpos, fadeDist, uv);
	#else
	float atten = SunshineLightAttenuation(mul(sunshine_WorldToSunVP, float4(wpos,1)));
	#endif
