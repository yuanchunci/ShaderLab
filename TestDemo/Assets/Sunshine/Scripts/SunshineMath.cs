...
public static class SunshineMath
{
	...
	public static void SetupShadowCamera(Light light, Camera lightCamera, Camera eyeCamera, float eyeNearClip, float eyeFarClip, float paddingZ, float paddingRadius, int snapResolution, ref BoundingSphere totalShadowBounds, ref ShadowCameraTemporalData temporalData)
	{
		...
		{
			paddingZ = eyeCamera.farClipPlane;
		...
		lightCamera.farClipPlane = totalShadowBounds.radius * 2f + paddingZ + lightCamera.nearClipPlane;
		lightCamera.orthographicSize = maxDimension2D * 0.5f;

		lightCamera.transform.rotation = Quaternion.LookRotation(light.transform.forward);
		
		lightCamera.transform.position = bounds.origin;
		
		//Quantize:
		Vector3 lightWorldOrigin = lightCamera.transform.InverseTransformPoint(Vector3.zero);
		...
		lightCamera.transform.position -= lightCamera.transform.TransformPoint(lightWorldOrigin);
		...
		Vector3 shadowCenterInLight = lightCamera.transform.InverseTransformPoint(totalShadowBounds.origin);
		lightCamera.transform.position += lightCamera.transform.forward * (shadowCenterInLight.z - (totalShadowBounds.radius + lightCamera.nearClipPlane + (Sunshine.Instance.UsingCustomBounds ? 0f : paddingZ)));
	}
